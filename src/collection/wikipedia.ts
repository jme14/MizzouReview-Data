...
import {AIPromptAnswers, Professor} from 'mizzoureview-reading/models/professor';
...
export async function setProfessorFunFacts(profArray: Professor[]): Promise<boolean>{
    for ( let i = 0 ; i < profArray.length ; i++){
        ...
        if (funFacts !== undefined){
            if (prof.aIPromptAnswers === undefined){
                prof.aIPromptAnswers = new AIPromptAnswers({funFacts: funFacts})
            } else {
                prof.aIPromptAnswers.funFacts = funFacts
            }
        }
    }
    return true
