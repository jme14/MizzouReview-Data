...
export async function urlValid(browser: Browser, page: Page, urlContains: RegExp) {
    const URL = page.url()
...
export async function navigateToFirstProfPage(
    ...
    let allNameElements = await page
        .getByText(nameregex)
        .all();
...
export async function getExpectedRatings(browser: Browser, page: Page){
    ...
    const elem = page.getByRole("link").filter({hasText: /^\d+\s+ratings$/})
